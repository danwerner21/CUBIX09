...
HEX_D           = 8                               ;START OF HEX DISPLAY
TEXT_D          = 60                              ;START OF TEXT
;*
...
ECMD3A
        LDX     #DOSERR                           ;INDICATE DOS ERROR
        STX     ERRMSG                            ;SET IT
ECMD3B
        JMP     VMODE2                            ;RETURN TO MAIN SCREEN
;* 'Q'UIT COMMAND
ECMD4
        CMPA    #'Q'                              ;QUIT?
...
SEARCH
        ...
        ABX                                       ;	ADVANCE IT
SEARC1
        LDY     #FNDBUF                           ;POINT TO FIND BUFFER
        CLRB                                      ;	ZERO COUNT
        LEAX    1,X                               ;ADVANCE BY 1
SEARC2
        CMPX    ENDADR                            ;ARE WE OVER?
...
SEARC3
        LDX     #NOTFND                           ;POINT TO MESSAGE
        JMP     VERROR                            ;REPORT ERROR
;*
ECMD8
        LDX     #BADCMD                           ;INVALID COMMAND
        JMP     VERROR                            ;REPORT ERROR
;*
;* DATA ENTRY KEY PRESSED
;*
ENTKEY
        LDB     MODE                              ;GET DATA MODE
...
DSCREEN
        ...
        FCB     33                                ;DISPLAY
DSCR1
        LDD     #$0004                            ;NEW POSITION
        JSR     GOTOXY                            ;POSITION CURSOR
        LDB     #16                               ;16x16 BYTE LINES
        TFR     U,X                               ;'X' = ADDR
DSCR2
        BSR     DLINE                             ;DISPLAY LINE
...
GETK2
        LDX     #GETKBUF	POINT TO KEY BUFFER
GETK3
        LDA     ,X+                               ;GET CHAR
...
HOMEMSG
        FCN     'Backup beyond start of file'
ENDMSG
        FCN     'Advance beyond end of file'
;* GETKEY INPUT BUFFER
GETKBUF
...
;*
ENDADR
        FDB     D_BUFF                            ;ENDING ADDRESS
CHANGED
        FCB     0                                 ;FILE CHANGED FLAG
POSIT
        FCB     0                                 ;CURSOR POSITION
MODE
        FCB     0                                 ;TEXT/HEX MODE
ERRMSG
        FDB     0                                 ;REPORT ERROR
TEMP
        RMB     1                                 ;TEMPORARY STORAGE
CMDBUF
        RMB     50                                ;COMMAND INPUT BUFFER
FNDBUF
        RMB     50                                ;FIND BUFFER
D_BUFF          = *                               ;DATA BUFFER
