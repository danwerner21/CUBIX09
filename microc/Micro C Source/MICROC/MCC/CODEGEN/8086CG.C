/*
 * DDS MICRO-C Code Generator for: 8086 Family
 *
 ...
 * Memory model:
 *
 * 0: Tiny	- combined code/data
 * 1: Small - split code/data, data pre-loaded
 * 2: Rom	- split code/data, data copied from code
 */
char model = 0;
	...
	int segment;
{
	if(segment != current_segment) {
		out_str("$SE:");
		out_num(current_segment = segment);
		out_nl(); }
}
...
/*
 * Test for operand on the stack & generate appropriate pops
 */
test_stack(token)
	unsigned token;
{
	...
	unsigned oper, rtype;
{
	...
	switch(oper) {
		...
		case _TO_TEMP:		/* copy accumulator to temp */
			ptr = byte ? "MOV ?temp,AL" : "MOV ?temp,AX" ;
			zflag = 0x55;
			break;
