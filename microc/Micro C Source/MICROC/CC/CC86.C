 ...
 *
 * This program integrates the MCP, MCC86, MCO86, SLINK, MACRO and ASM86
...
char htext[] = { "\n\
Use: CC86 <name> [-acdfiklmopqsx h= m= s= t=] [symbol=value]\n\n\
opts:	-Asm		-Comment	-Dupwarn	-Foldliteral\n\
	-Intelhex	-Keeptemp	-Listing	-Macro\n\
	-Optimize	-Preprocess	-Quiet		-Symbolic\n\
	-eXtended-asm\n\n\
	H=homepath	M=TSR(model)	S=startup	T=temprefix\n\
\n\?COPY.TXT 1990-2005 Dave Dunfield\n**See COPY.TXT**.\n" };

int model = 0;
char *mlib[] = { "TINY", "SMALL", "ROM" };
	...
	int *argv[];
{
	...
	message("DDS MICRO-C 8086 Cross Compiler v3.23\n");
...
noext:
	...
	*mptr = *ptr = 0;
	...
	if(link) {
		...
		sprintf(tail, "%s %s t=%s l=%s\\LIB86 i=%s.LIB -q%s%s",
