 ...
 *
 * This program integrates the MCP, MCC16, MCO16, SLINK, MACRO and ASM16
...
char link = -1, opt = 0, pre = 0, xasm = -1, verb = -1, intel = 0,
	lst = 0, del = -1, com = 0, macro = 0, fold = 0, symb = 0, dup = 0,
	*fnptr, *mptr = &mcparm, *startup = 0;

char htext[] = { "\n\
Use: CC16 <name> [-acdfiklmopqsx h= m= s= t=] [symbol=value]\n\n\
opts:	-Asm		-Comment	-Dupwarn	-Foldliteral\n\
	-Intelhex	-Keeptemp	-Listing	-Macro\n\
	-Optimize	-Preprocess	-Quiet		-Symbolic\n\
	-eXtended-asm\n\n\
	H=homepath	M=SL(model)	S=startup	T=temprefix\n\
\n\?COPY.TXT 1990-2005 Dave Dunfield\n**See COPY.TXT**.\n" };

int model = 0;
char *mlib[] = { "SMALL", "LARGE" };
	...
	int *argv[];
{
	...
	message("DDS MICRO-C 68HC16 Cross Compiler v3.23\n");
...
noext:
	...
	*mptr = *ptr = 0;
	...
	if(link) {
		...
		sprintf(tail, "%s %s t=%s l=%s\\LIB16 i=%s.LIB -q -w%s%s",
			ifile, ofile, temp, mcdir, mlib[model], symb ? " -s" : "", mcparm);
		docmd("SLINK.EXE");
		erase(ifile);
		strcpy(ifile, ofile);
